            ...
            if imgs_ is not None and imgs_ != False:
                                ...
                                if ganghwa == True:
                                    ...
                                    if is_item == True:
                                        ...
                                        for c in range(10):
                                            ...
                                            img_array = np.fromfile(full_path, np.uint8)
                                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                            imgs_ = imgs_set_(200, 270, 465, 335, cla, img, 0.85)
                                            if imgs_ is not None and imgs_ != False:
                                                is_upgrade = False
                                                if upgrade == 5:
                                                    is_upgrade = True
                ...
                for i in range(10):
                    ...
                    if imgs_ is not None and imgs_ != False:
                        ...
                        full_path = "c:\\my_games\\lordnine\\data_lordnine\\imgs\\action\\menu\\menu_collection.PNG"
...
def boonhae_start(cla):
    ...
    try:
        ...
        elif v_.onCollection_boonhae == True:
            boonhae_option(cla, "high")
    ...
    try:
        ...
        while boonhae_option is False:
            ...
            if imgs_ is not None and imgs_ != False:
                ...
                img_array = np.fromfile(full_path, np.uint8)
                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                imgs_ = imgs_set_(260, 370, 315, 415, cla, img, 0.7)
                if imgs_ is not None and imgs_ != False:
                    # 고급까지 체크인지 확인
                    full_path = "c:\\my_games\\lordnine\\data_lordnine\\imgs\\boonhae_collection\\boonhae_checked.PNG"
                    img_array = np.fromfile(full_path, np.uint8)
                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                    imgs_ = imgs_set_(360, 370, 420, 415, cla, img, 0.7)
                    if imgs_ is not None and imgs_ != False:
                        ...
                        if data == "rare":
                            ...
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(470, 370, 515, 415, cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                ...
                                img_array = np.fromfile(full_path, np.uint8)
                                img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                imgs_ = imgs_set_(560, 370, 625, 415, cla, img, 0.7)
                                if imgs_ is not None and imgs_ != False:
                                    check_end = False
                                else:
                                    full_path = "c:\\my_games\\lordnine\\data_lordnine\\imgs\\boonhae_collection\\boonhae_checked.PNG"
                                    img_array = np.fromfile(full_path, np.uint8)
                                    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                                    imgs_ = imgs_set_(260, 400, 420, 435, cla, img, 0.7)
                                    if imgs_ is not None and imgs_ != False:
                                        check_end = False
                                    else:
                                        check_end = True
                            ...
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(460, 370, 625, 415, cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                check_end = False
                            else:
                                full_path = "c:\\my_games\\lordnine\\data_lordnine\\imgs\\boonhae_collection\\boonhae_checked.PNG"
                ...
                if check_end == True:
                    ...
                    for i in range(5):
                        ...
                        if imgs_ is not None and imgs_ != False:
                            ...
                            img_array = np.fromfile(full_path, np.uint8)
                            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
                            imgs_ = imgs_set_(380, 650, 550, 760, cla, img, 0.7)
                            if imgs_ is not None and imgs_ != False:
                                click_pos_reg(imgs_.x, imgs_.y, cla)
                        else:
                            break
